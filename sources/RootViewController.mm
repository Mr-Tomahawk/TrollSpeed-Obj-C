...
#import "SPLarkController/SPLarkTransitioningDelegate.h"
...
@interface RootViewController () <TSSettingsControllerDelegate>

@property (strong, nonatomic) SPLarkTransitioningDelegate *customTransitioningDelegate;
...
- (BOOL)settingHighlightedForKey:(NSString * _Nonnull)key
{
    [self loadUserDefaults:NO];
...
- (void)settingDidSelectForKey:(NSString * _Nonnull)key
{
    BOOL highlighted = [self settingHighlightedForKey:key];
...
- (void)tapSettingsButton:(UIButton *)sender
{
    ...
    self.customTransitioningDelegate = [[SPLarkTransitioningDelegate alloc] init]; 
    settingsViewController.transitioningDelegate = self.customTransitioningDelegate;
