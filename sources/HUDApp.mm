...
int main(int argc, char *argv[])
{
    @autoreleasepool
    {
        ...
        if (strcmp(argv[1], "-hud") == 0) {
            ...
            [pidString writeToFile:ROOT_PATH_NS(PID_PATH) atomically:YES encoding:NSUTF8StringEncoding error:nil];
            ...
            NSString *pidString = [NSString stringWithContentsOfFile:ROOT_PATH_NS(PID_PATH) encoding:NSUTF8StringEncoding error:nil];
        ...
        NSDictionary *prefs = [NSDictionary dictionaryWithContentsOfFile:ROOT_PATH_NS(@"/var/mobile/Library/Preferences/ch.xxtou.trollspeed.plist")];
        return (prefs != nil) ? EXIT_SUCCESS : EXIT_FAILURE;
#else
        return UIApplicationMain(argc, argv, @"MainApplication", @"MainApplicationDelegate");
#endif
    }
